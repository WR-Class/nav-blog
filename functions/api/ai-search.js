...
async function scrape(url, itemSel, snippetSel, unwrap) {
  ...
  await new HTMLRewriter()
    .on(itemSel, {
      element(el) {
        ...
        current = href ? { url: href, title: "", snippet: "" } : null;
...
function searchBing(q) {
  const url =
    "https://www.bing.com/search?q=" + encodeURIComponent(q) + "&setlang=zh-CN&count=15";
  return scrape(url, "li.b_algo h2 a", "li.b_algo p", bingUnwrap);
}

function searchDDG(q) {
  const url = "https://lite.duckduckgo.com/lite/?q=" + encodeURIComponent(q);
  return scrape(url, "a.result-link", "td.result-snippet", ddgUnwrap);
}

function searchMojeek(q) {
  const url = "https://www.mojeek.com/search?q=" + encodeURIComponent(q);
  return scrape(url, "a.ob", "p.s", (h) => h || "");
}
...
async function searchWeb(query) {
  ...
  for (const variant of expandQueries(query)) {
    let results = [];
    for (const engine of [searchBing, searchDDG, searchMojeek]) {
      try {
        results = await engine(variant);
      } catch {
        results = [];
      }
      if (results.length) break; // 某源可用就不再打扰其它源
    }
...
async function generateAnswer(query, pages, env) {
  ...
  if (!env.AGNES_API_KEY) {
    return "已找到相关来源，但站点尚未配置 AI 模型密钥（AGNES_API_KEY）。可先查看下方来源。";
  }
  ...
  try {
      ...
      AGNES_API_URL,
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: buildPrompt(query, pages) },
          ],
        }),
      },
      60000
    ...
    if (!resp.ok) {
      return `模型服务暂时不可用（HTTP ${resp.status}）。下方是检索到的来源，可直接查看。`;
    }
    const data = await resp.json();
    return (
      data?.choices?.[0]?.message?.content ||
      "模型未返回内容。下方是检索到的来源，可直接查看。"
    );
  } catch {
    return "模型调用超时或失败。下方是检索到的来源，可直接查看。";
  }
}
